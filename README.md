...
* Запись в установленный путь.
* Доступна обычная функция `modify({json: '...'}).toValue() -> {...}`.
...
const nodeRoot = modify(options: Options): INodeRoot
// Чтение и запись файла.
const rwNodeRoot = rwModify (options: Options, src?: string, dest?: string): INodeRoot
```
...
Ошибки разбора _Json_ или чтения файла, всегда являются фатальными и не возвращают ожидаемый результат. Функции не выбрасывают исключений, проверять наличие ошибок следует через интерфейс `INodeRoot`. В случае `isFatalError`, текущий результат будет потерен и `toJson()` или `toValue()`, будут возвращать `""` или `null`.

> 📌 Не устанавливайте `error` при преобразовании `JS`-объекта, где могут быть `function`, `undefined` и `symbol`. В этом случае невалидные значения игнорируются и ошибка не устанавливается, в том числе и для ошибок поиска путей. Список всех ошибок всегда доступен через `INodeRoot.errors:IErrors`.
